 ...
 */
void computeGridSize(const struct settings * simsetup, const struct state * simstate, struct gridData *grid)
{
  doubleVector3d globalGridSize;

  grid->resolution = csize * simsetup->gfH;
  grid->voxel_volume = pow((grid->resolution / csize) * csizeInUnits * 0.0001, 3);
  ...
  grid->global_size.x = grid->global_size.x + (simstate->MPIdim[0] - grid->global_size.x % simstate->MPIdim[0]);
  grid->global_size.y = grid->global_size.y + (simstate->MPIdim[1] - grid->global_size.y % simstate->MPIdim[1]);
  if (simsetup->dimension == 3)
    grid->global_size.z = grid->global_size.z + (simstate->MPIdim[2] - grid->global_size.z % simstate->MPIdim[2]);
 ...
 */
void allocateGrid(const struct settings * simsetup, const struct state * simstate,struct gridData *grid)
{
  int i, j, k;
  #define grid_node(i,j,k) (grid->buffer[grid->local_size.y*grid->local_size.z*i+grid->local_size.z*j+k])
  grid->lower_indices =
      (struct int64Vector3d *) malloc(simstate->MPIsize *
  ...
  for (i = 0; i < simstate->MPIsize; i++) {
    ...
    grid->upper_indices[i].x = grid->lower_indices[i].x + grid->local_size.x - 1;
    grid->upper_indices[i].y = grid->lower_indices[i].y + grid->local_size.y - 1;
    if (simsetup->dimension == 3)
      grid->upper_indices[i].z = grid->lower_indices[i].z + grid->local_size.z - 1;
    else
  ...
  if (!
      (grid->buffer =
       (doubleVector3d *) calloc((size_t) grid->local_size.x * grid->local_size.y *
                                        grid->local_size.z,
  ...
  for (i = 0; i < grid->local_size.x; i++)
    for (j = 0; j < grid->local_size.y; j++)
      for (k = 0; k < grid->local_size.z; k++) {
        grid_node(i, j, k).x =
            grid->lower_corner.x + grid->resolution * (grid->lower_indices[simstate->MPIrank].x +
                                                  i);
        grid_node(i, j, k).y =
            grid->lower_corner.y + grid->resolution * (grid->lower_indices[simstate->MPIrank].y +
                                                  j);
        grid_node(i, j, k).z =
            grid->lower_corner.z + grid->resolution * (grid->lower_indices[simstate->MPIrank].z +
                                                  k);
      }
...
void envInit(size_t nnutrients) {
 ...
 */
void envInitSystem(const struct state * simstate, const struct settings *set, struct gridData *grid)
{
  int i, j, k, c;
  int entry;
  size_t var;
  ...
  HYPRE_SStructGridSetVariables(envGrid, 0, (HYPRE_Int) set->numberOfEnvironments, envVarTypes);
  ...
  nvalues = (size_t) nentries * grid->local_size.x * grid->local_size.y * grid->local_size.z;
  ...
  for(var=0;var<set->numberOfEnvironments;var++) {
    ...
    envZ[var] =
            set->environments[var].diffusion_coefficient * dt[var] / (gridResolutionInUnits *
                                      gridResolutionInUnits);

    // set the standard stencil at each grid point,
    //  we will fix the boundaries later
    for (i = 0; i < nvalues; i += nentries) {
      values[i] = 1 + 6.0 * envZ[var] + set->environments[var].lambda_delay * dt[var];
 ...
 */
void envCellPC(const struct state * simstate, double *envPC, int var, struct gridData *grid)
{
 ...
 */
void envInitBC(const struct state * simstate, const struct settings *set, struct gridData * grid)
{
  HYPRE_Int i, j, k;
  int mi;
  int var;
 ...
 */
void envSolve(const struct state * simstate, const struct settings * simsetup, struct gridData * grid)
{
  size_t i, j, k;
  int idx;
  int var;
  double *values;
  int stepIter = 0;
  size_t nvalues = (size_t) grid->local_size.x * grid->local_size.y * grid->local_size.z;
  double *envPC;
  if (simstate->MPIrank == 0 ) {
  ...
  while (stepIter < numberOfIters) {
    if (envIter > 0) {
      ...
      HYPRE_SStructVectorSetBoxValues(b, 0, envLower, envUpper,
                                      var, values);
      for (i = 0; i < nvalues; i++)
        values[i] = envZ[var] * simsetup->environments[var].boundary_condition;
      if (simstate->MPIcoords[simstate->MPIrank][0] == 0) {
        envSetBoundary(0, -1);
...
void allocateFields(const struct settings * simsetup, struct gridData * grid) {
  ...
  fieldAddr=(double**)malloc(simsetup->numberOfEnvironments*sizeof(double*));
  envField=(double**)malloc(simsetup->numberOfEnvironments*sizeof(double*));

  fieldConsumption=(double*)malloc(simsetup->numberOfEnvironments*sizeof(double));
  fieldProduction=(double*)malloc(simsetup->numberOfEnvironments*sizeof(double));
...
void computeEnvironment(const struct settings * simsetup, struct state * simstate, struct gridData * grid) {
  double t0,t1;
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  t1=MPI_Wtime();
  if(simstate->MPIrank==0) printf("TIME: %f\n",t1-t0);
}

void prepareEnvironment(struct settings *set){
  size_t i;
  set->environments = calloc(set->numberOfEnvironments, sizeof(struct environment));
...
int main(int argc,char* argv[]) {
  ...
  periods[2] = 0;
  reorder = 0;
  MPI_Cart_create(MPI_COMM_WORLD, simsetup.dimension, simstate.MPIdim, periods, reorder,
                  &(simstate.MPI_CART_COMM));
  simstate.MPIcoords = (int **) malloc(simstate.MPIsize * sizeof(int *));
  for (i = 0; i < simstate.MPIsize; i++) {
    simstate.MPIcoords[i] = (int *) malloc(3 * sizeof(int));
    MPI_Cart_coords(simstate.MPI_CART_COMM, i, simsetup.dimension, simstate.MPIcoords[i]);
  }
  /* koniec */
